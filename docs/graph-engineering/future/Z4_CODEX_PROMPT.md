# Z4 entry prompt — inspectable evidence and deterministic tools

Use this prompt only when the user explicitly selects Z4. Merely reading/copying the future pack does not authorize implementation or later stages.
...
Review the final diff and fix in-scope defects. Write docs/graph-engineering/Z4_REPORT.md using the shared template, update current progress/specs, and provide actual screenshots and exact manual instructions. If required native checks are missing, report that accurately rather than unqualified readiness.

Stop after Z4. Identify the next eligible milestone but do not begin it.
